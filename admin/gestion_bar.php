...
{	
	...
	<a href="'.RACINE_SITE.'admin/gestion_promo_bar.php"> > Offres Apéro</a>';
}
...
else
{
	echo '<h2><a href="?affichage=affichage" class="button" >Tous les bars</a></h2>
		...
		<h2><a href="'.RACINE_SITE.'admin/gestion_promo_bar.php">Offres Apéro</a><h2>';
}
	...
	if(isset($_GET['action']) &&  $_GET['action']=='ajout') 
		{
			...
			<fieldset>
				...
				<input type="hidden" name="id_bar" id="id_bar" value="<?php if(isset($bar_actuel['id_bar'])){ echo $bar_actuel['id_bar']; }?>" /><!-- On met un input caché pour pouvoir identifier le membre lors de la modification (REPLACE se base sur l'id uniquement(PRIMARY KEY)) /!\SECURITE : On est ici dans un back-office, on peut donc se permettre une certaine confiance en l'utilisateur, mais les champs cachés ne sont pas sécurisés pour l'acces public il faut faire des controles securités sur les url -->
					...
					<input required type="text" id="nom_bar" name="nom_bar"  maxlength="14" value="<?php if(isset($_POST['nom_bar'])) {echo $_POST['nom_bar'];}?>" placeholder="Puzzle Bar" /><br />
					...
					<textarea id="description" name="description" class="description_form" ><?php if(isset($_POST['description'])) {echo $_POST['description'];} ?></textarea>
					
					<label for="nom">Nom du gérant</label>
					<input required type="text" id="nom" name="nom" value="<?php if(isset($_POST['nom'])) {echo $_POST['nom'];}  if(isset($_GET['id_membre'])){ echo $membre['nom'];}?>" placeholder="Durand" required /><br />
					
					<label for="prenom">Prénom du gérant</label>
					<input required  type="text" id="prenom" name="prenom" value="<?php if(isset($_POST['prenom'])) {echo $_POST['prenom'];} if(isset($_GET['id_membre'])){ echo $membre['prenom'];}?>" placeholder="Jean"  required /><br />
					
					<h3>Coordonnées de l'établissement</h3>
					<label for="email">Email</label>
					<input required  type="email" id="email" name="email" value="<?php if(isset($_POST['email'])) {echo $_POST['email'];}?>" placeholder="monmail@mail.com"  required /><br />
		
					<label for="cp">Téléphone</label>
					<input required  type="text" id="telephone" name="telephone" value="<?php if(isset($_POST['telephone'])) {echo $_POST['telephone'];}?>" placeholder="O111223344" required/><br />
