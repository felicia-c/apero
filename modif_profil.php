...
if($_POST)
{
 // SECURITE 
	...
	{
		$msg .= '<div class="msg_erreur" ><h4>Pseudo invalide - Caractères acceptés: A à Z et 0 à 9</h4></div>';  
	}
	$verif_caractere = preg_match('#^[a-zA-Z0-9._-]+$#', $_POST['old_mdp']);
	if(!$verif_caractere && !empty($_POST['old_mdp']))
	{
		$msg .= '<div class="msg_erreur" ><h4>Mot de passe invalide - Caractères acceptés: -_ A à Z et 0 à 9</h4></div>';  
	}
	$verif_caractere = preg_match('#^[a-zA-Z0-9._-]+$#', $_POST['mdp']);
	if(!$verif_caractere && !empty($_POST['mdp']))
	{
		$msg .= '<div class="msg_erreur" ><h4>Nouveau mot de passe invalide - Caractères acceptés: -_ A à Z et 0 à 9</h4></div>';  
	}
	
	$verif_caractere = preg_match('#^[a-zA-Z0-9._-]+$#', $_POST['mdp2']);
	if(!$verif_caractere && !empty($_POST['mdp2']))
	{
		$msg .= '<div class="msg_erreur" ><h4></h4></div>';  
	}
	
	$verif_caractere = preg_match('#^[àâäçéèêëïa-zA-Z0-9._ \'-]+$#', $_POST['nom']); 
	if(!$verif_caractere && !empty($_POST['nom']))
	{
		$msg .= '<div class="msg_erreur" ><h4>Nom invalide - Caractères acceptés: - A à Z et 0 à 9</h4></div>';  
	}
	
	$verif_caractere = preg_match('#^[àâäçéèêëïa-zA-Z0-9._ -]+$#', $_POST['prenom']); 
	if(!$verif_caractere && !empty($_POST['prenom']))
	{
		$msg .= '<div class="msg_erreur" ><h4>Prenom invalide - Caractères acceptés: àâäçéèêëï - A à Z et 0 à 9</h4></div>';  
	}
	...
	{
		$msg .= '<div class="msg_erreur" ><h4>Ville invalide - Caractères acceptés: àâäçéèêëï, A à Z -_ 0 à 9 </h4></div>';  
	}
	
	$verif_caractere = preg_match('#^[àâäçéèêëïa-zA-Z0-9.,_ \'-]+$#', $_POST['adresse']); 
	if(!$verif_caractere && !empty($_POST['adresse']))
	{
		$msg .= '<div class="msg_erreur"><h4>Adresse invalide - Caractères acceptés: àâäçéèêëï A à Z et 0 à 9</h4></div>';  
	}
	
	$verif_caractere = intval($_POST['cp']); 
	if(!$verif_caractere && !empty($_POST['cp']))      
	{
		$msg .= '<div class="msg_erreur" ><h4>Code postal invalide</h4></div>';  
	}
	...
	{
		$msg .= '<div class="msg_erreur" ><h4>Le nom doit avoir entre 2 et 45 caractères inclus</h4></div>';
	}
	if(strlen($_POST['prenom'])< 1 || strlen($_POST['prenom'])>45) 
	{
		$msg .= '<div class="msg_erreur"><h4>Le prénom doit avoir entre 1 et 45 caractères inclus</h4></div>';
	}
...
?>
	...
	<form class="form" method="post" action="" enctype="multipart/form-data"> <!--enctype pour ajout eventuel d'un champs photo -->
		...
		<fieldset>
			...
			<input type="password" id="old_mdp" name="old_mdp"  maxlength="15" value="<?php if(isset($_POST['mdp'])) {echo $_POST['mdp'];}?>"  placeholder="" /><br /><br />
			
			<label for="mdp">Nouveau mot de passe</label><br />
			<input type="password" id="mdp" name="mdp"  maxlength="15" value="<?php if(isset($_POST['mdp'])) {echo $_POST['mdp'];}?>"  placeholder=""  /><br /><br />				
			...
			<input type="text" id="nom" name="nom" maxlength="45" value="<?php if(isset($membre_actuel['nom'])) {echo $membre_actuel['nom'];} elseif(isset($_POST['nom'])){ echo $_POST['nom'];}?>" placeholder="Durand" required /><br />
			
			<label for="prenom">Prénom</label>
			<input type="text" id="prenom" name="prenom" maxlength="45" value="<?php if(isset($membre_actuel['prenom'])) {echo $membre_actuel['prenom'];} elseif(isset($_POST['prenom'])){ echo $_POST['prenom'];}?>" placeholder="Jean"  required /><br />
			
			
			<label for="email">Email</label>
			<input type="text" id="email" name="email" maxlength="45" value="<?php if(isset($membre_actuel['email'])) {echo $membre_actuel['email'];} elseif(isset($_POST['email'])){ echo $_POST['email'];}?>" placeholder="monmail@mail.com"  required /><br />
